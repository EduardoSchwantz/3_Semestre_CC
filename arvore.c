...
No* rotacaoEsquerda(No *raiz){ // função retorna um ponteiro do tipo no, que sera nossa nova raiz.
...
No* rotacaoDireita(No *raiz){   // função retorna um ponteiro do tipo no, que sera nossa nova raiz.
    No *novaraiz, *filho;       // criasse dois ponteiros auxiliares para salvar endereços da nova raiz e do possivel filho.
...
No* remover(No *raiz, int valor) {
    if(raiz == NULL){
        ...
        if(raiz->valor == valor) {  // procura o nó a remover
            if(raiz->esquerdo == NULL && raiz->direito == NULL) {   // remove nós folhas (nós sem filhos)
                free(raiz);
