											...
											/* chamada head  e um ponteiro do tipo char chamada text*/
{
	...
	for (p = head; p != NULL; p = p -> next) {		/* inicia um for que p recebe head, repete ate p for diferente de nulo, e tem como incremento p recebe struct p campo next.*/
            if (strcmp(text, p -> item) == 0) {  /* se a comparação de text e struct p no campo item forem iguais, faça*/
...
THING *addmiddle (THING *head, THING *newp)	/*cria uma função ponteiro do tipo thing chamada addmiddle, recebe como parametro dois ponteiro do tipo thing, head e newp */
{
	BOOL found = FALSE;
	THING *p1, *p2; 	
	if (head == NULL) { /* caso especial se a lista for vazia. */
		head = addfront(head, newp);
		return head;
	}
	p2 = p1 = head ; /* p1 recebe head e p2 recebe p1. */
	while (!found) {  /* repita ate for diferente de found. "( 0 )" */
  		if (found = strcmp(p1 -> item, newp -> item) >= 1) { /* se found for igual a comparação de struct campo item a struct newp campo item e a comparação maior ou igual a 1 faça */
			if (p1 == head) { /* se p1 for igual a head faça */
//					adiciona na frente
					head = addfront(head, newp); 
					return(head);
		        }
				...
				p2 -> next = newp;; /* struct p2 campo next recebe newp. */
				newp -> next = p1; /*struct newp no campo next recebe p1 */
				return head;
	  		}
	  	}

	if  (p1 -> next == NULL) { // caso não encontre o inicio da lista adiciona no fim
...
// usa a tecnica de ponteiros duplos para imprimir a lista.
{
	...
	while ((*tracer) != NULL) { /* repita ate *tracer for diferente de nulo,  seja repita até o proximo no for nulo*/
		printf("%s \n",(*tracer)->item);
